...
A software CPU (instruction-set simulator) written in C, suitable for electronics/CS labs. Implements a deterministic, reproducible CPU simulator with a custom 8-bit ISA, assembler, disassembler, monitor/debugger, and test suite.
...
### Build
```bash
# Configure (choose the line that matches your platform)
...
### Run Examples
```bash
# Run CPU simulator (binaries land in build/build/)
build/build/cpu-sim examples/hello.bin --addr 0x0200 --run
...
### Test Suite
```bash
build/build/tests        # Linux / macOS
build\build\tests.exe   # Windows
```
...
# Build example programs (assembles .asm → .bin)
...
The project includes a comprehensive test suite. Run the test executable directly — ctest is not configured:
...
- **Assembler does not recognise `LDI`**: all three example programs (`hello.asm`, `addloop.asm`, `gpio_blink.asm`) fail to assemble with `Unknown instruction: DI`. The assembler mis-parses the `LDI` mnemonic regardless of the operand. The CPU core, ISA, and test suite all work correctly; the issue is isolated to the assembler's mnemonic lookup.

## Contributing

The codebase follows C99 standards and is designed to be portable across different platforms.
